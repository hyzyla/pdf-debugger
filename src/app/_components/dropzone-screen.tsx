...
function Dropzone(props: { onDrop: (file: Blob) => void }) {
  ...
  return (
    <div {...getRootProps()} className={cn(DROPZONE_COMMON_CLASSES, "border-green-700")}>
      <input {...getInputProps()} />
...
function ExampleButton(props: { onExample: () => void }) {
  return (
    <div className={cn(DROPZONE_COMMON_CLASSES, "border-gray-400")} onClick={props.onExample}>
...
export function DropzoneScreen(props: { onDrop: (file: Blob) => void; onExample: () => void }) {
  return (
    <div className="flex-1 flex flex-col gap-6 overflow-y-auto max-w-[40rem]">
      <p className="">This tool allows you to inspect tree structure of a PDF file:</p>
